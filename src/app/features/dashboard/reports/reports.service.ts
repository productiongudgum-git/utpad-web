...
export class ReportsService {
  ...
  async fetchProduction(from: string, to: string): Promise<ProductionReportRow[]> {
    const { data } = await this.supabase.client
      ...
      .select('batch_code, production_date, planned_yield, actual_yield, worker_id, sku:gg_flavors!production_batches_sku_id_fkey(name), recipe:gg_recipes(title)')
    ...
    return ((data as any[]) ?? []).map((r: any) => ({
      ...
      recipe: r.recipe?.title ?? r.recipe_id,
  ...
  async fetchPacking(from: string, to: string): Promise<PackingReportRow[]> {
    const { data } = await this.supabase.client
      ...
      .select('batch_code, session_date, boxes_packed, worker_id, flavor_id, flavor:gg_flavors!packing_sessions_flavor_id_fkey(name)')
      .gte('session_date', from)
      .lte('session_date', to)
      .order('session_date', { ascending: false });
    return ((data as any[]) ?? []).map((r: any) => ({
      batch_code: r.batch_code,
      sku: r.flavor?.name ?? r.flavor_id ?? '',
  ...
  async fetchDispatch(from: string, to: string): Promise<DispatchReportRow[]> {
    const { data } = await this.supabase.client
      ...
      .select('invoice_number, customer_name, batch_code, boxes_dispatched, dispatch_date, worker_id, sku:gg_flavors(name)')
  ...
  async fetchInventorySnapshot(): Promise<InventoryReportRow[]> {
    const { data: rm } = await this.supabase.client
      ...
      .select('current_qty, unit, low_stock_threshold, ingredient:gg_ingredients(name)');
    const { data: fg } = await this.supabase.client
      .from('inventory_finished_goods')
      .select('boxes_available, sku:gg_flavors(name)');
  ...
  async fetchReturns(from: string, to: string): Promise<ReturnsReportRow[]> {
    const { data } = await this.supabase.client
      .from('returns_events')
      .select('batch_code, qty_returned, reason, return_date, worker_id, sku:gg_flavors(name)')
